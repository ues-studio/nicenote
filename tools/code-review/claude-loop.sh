...
# 支持：收敛检测自动退出 | 轮间 Git 提交 | 维度轮换聚焦 | 中断安全
...
#   ./tools/code-review/claude-loop.sh . 5 --dry-run   # 只审查不修复
##############################################################################
...
commit_round() {
  ...
  # 使用 git -C 避免 cd 改变主进程工作目录
  if git -C "$PROJECT_DIR" diff --quiet && \
     git -C "$PROJECT_DIR" diff --cached --quiet && \
     [ -z "$(git -C "$PROJECT_DIR" ls-files --others --exclude-standard --directory)" ]; then
    log_info "本轮无代码变更，跳过提交"
    return 0
  fi

  # 暂存所有变更（包括 Claude 新建的文件），但排除报告目录
  git -C "$PROJECT_DIR" add -A -- ':!.arch-review'
  git -C "$PROJECT_DIR" commit -m "$(cat <<EOF
chore(review): 第 ${round} 轮自动审查修复

Co-Authored-By: Claude Opus 4.6 (1M context) <noreply@anthropic.com>
EOF
  )" || {
    log_warn "Git 提交失败，继续执行"
    return 0
  }
  log_ok "第 ${round} 轮变更已提交"
}
