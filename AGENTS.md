...
packages/
  domain/         # 领域层：Repository 接口 + Use Case 工厂（纯 TS，零 I/O）
  editor/         # Tiptap v3 富文本编辑器组件
  ui/             # Radix UI 组件库（Button, Popover, Toolbar 等）
  tokens/         # Design tokens（颜色、字体、间距、阴影）
...
# Web (apps/web)
pnpm --filter web dev            # Vite 开发服务器 (port 5173)
pnpm --filter web build          # 构建（自动先生成 CSS tokens）
pnpm --filter web generate:css   # 从 design tokens 重新生成 CSS
...
```bash
pnpm --filter web generate:css   # 确保 CSS tokens 最新
...
Vitest workspace 模式，覆盖范围：`packages/editor`、`packages/shared`、`apps/web`。

```bash
pnpm test                        # 全量
pnpm --filter web test           # 单包
...
DDD 分层设计——纯 TypeScript 接口和 Use Case 工厂，零 I/O 依赖：

- **Repository 接口**：`NoteRepository`、`SearchIndex`、`SettingsRepository`
- **Use Cases**：`createCreateNoteUseCase`、`createLoadNotesUseCase`、`createSaveNoteUseCase`、`createSearchNotesUseCase`

各平台提供自己的 Repository 实现（Desktop 用文件系统，Mobile 用 SQLite，Web 待接入）。
...
Zustand store：`apps/web/src/store/useNoteStore.ts`。数据层已移除（原 Cloudflare Workers API），待接入新数据源。

### 主题系统

Design tokens 在 `packages/tokens/` 中定义，通过 CSS 生成脚本编译为 CSS 变量：

- Web：`apps/web/scripts/generate-css.ts`
- Desktop：`apps/desktop/frontend/scripts/generate-css.ts`
- Tokens 包自身：`packages/tokens/scripts/generate-css.ts`
...
Tiptap v3，入口 `packages/editor/src/index.ts`。扩展：StarterKit (含 Link)、TextAlign、Typography、Placeholder、Markdown。**内容始终以 Markdown 格式存储**。
...
- Store 文件使用 lazy `svc()` accessor（每次创建新 service 实例——无状态）
...
- 修改 store 时：Desktop 遵循 slice 模式，Web 使用单 store 模式。
