...
// --- UI Primitives ---
import { Button, type ButtonProps, Badge } from '@nicenote/ui'
      ...
      >
        {children ?? (
          <>
            <span
              className={cn(
                'w-5 h-5 mx-[-0.175rem] rounded-xl relative bg-[var(--highlight-color)] transition-transform duration-200',
                'after:content-[""] after:absolute after:inset-0 after:rounded-inherit after:border after:border-[var(--highlight-color)]',
                'after:brightness-95 after:mix-blend-multiply',
                'dark:after:brightness-140 dark:after:mix-blend-lighten',
                'group-data-[active-state=on]:after:brightness-80 dark:group-data-[active-state=on]:after:brightness-180',
                'tiptap-button-highlight'
              )}
              style={{ '--highlight-color': highlightColor } as React.CSSProperties}
            />
            {text && <span className={cn('flex-grow text-left leading-6 ml-1')}>{text}</span>}
