...
import { Button, cn } from '@nicenote/ui'
...
const CloudUploadIcon: React.FC = () => (
  <svg
    ...
    height="24"
    viewBox="0 0 24 24"
    className="tiptap-image-upload-icon w-3.5 h-3.5 text-white dark:text-black"
    fill="currentColor"
...
const FileIcon: React.FC = () => (
  <svg
    ...
    viewBox="0 0 43 57"
    fill="currentColor"
    className="tiptap-image-upload-dropzone-rect-primary text-muted absolute"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
...
const FileCornerIcon: React.FC = () => (
  <svg
    width="10"
    height="10"
    className="tiptap-image-upload-dropzone-rect-secondary absolute top-0 right-1 bottom-0 text-border"
    viewBox="0 0 10 10"
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
  >
...
const ImageUploadDragArea: React.FC<ImageUploadDragAreaProps> = ({ onFile, children }) => {
  ...
  return (
    <div
      className={cn(
        'tiptap-image-upload-drag-area p-8 border-1.5 border-dashed border-border rounded-md text-center cursor-pointer relative overflow-hidden transition-all hover:border-muted-foreground',
        isDragActive && 'border-primary bg-primary/5 drag-active',
        isDragOver && 'border-primary bg-primary/10 drag-over'
      )}
...
const ImageUploadPreview: React.FC<ImageUploadPreviewProps> = ({ fileItem, onRemove }) => {
  ...
  return (
    <div className="tiptap-image-upload-preview relative rounded-md overflow-hidden">
      {fileItem.status === 'uploading' && (
        <div
          className="tiptap-image-upload-progress absolute inset-0 bg-primary/10 transition-all duration-300"
          style={{ width: `${fileItem.progress}%` }}
        />
      )}

      <div className="tiptap-image-upload-preview-content relative border border-border rounded-md p-4 flex items-center justify-between">
        <div className="tiptap-image-upload-file-info flex items-center gap-3 h-8">
          <div className="tiptap-image-upload-file-icon p-2 bg-primary rounded-xl flex items-center justify-center">
            <CloudUploadIcon />
          </div>
          <div className="tiptap-image-upload-details flex flex-col">
            <span className="tiptap-image-upload-text text-foreground font-medium text-sm">
              {fileItem.file.name}
            </span>
            <span className="tiptap-image-upload-subtext text-muted-foreground font-semibold text-xs">
              {formatFileSize(fileItem.file.size)}
            </span>
          </div>
        </div>
        <div className="tiptap-image-upload-actions flex items-center gap-2">
          {fileItem.status === 'uploading' && (
            <span className="tiptap-image-upload-progress-text text-xs text-primary font-bold">
              {fileItem.progress}%
            </span>
          )}
          <Button
...
const DropZoneContent: React.FC<{ maxSize: number; limit: number }> = ({ maxSize, limit }) => (
  <>
    <div className="tiptap-image-upload-dropzone relative w-12 h-[60px] inline-flex items-start justify-center select-none">
      <FileIcon />
      <FileCornerIcon />
      <div className="tiptap-image-upload-icon-container absolute w-7 h-7 bottom-0 right-0 bg-primary rounded-xl flex items-center justify-center">
        <CloudUploadIcon />
      </div>
    </div>

    <div className="tiptap-image-upload-content flex items-center justify-center flex-col gap-1 select-none">
      <span className="tiptap-image-upload-text text-foreground font-medium text-sm">
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
  ...
  return (
    <NodeViewWrapper className="tiptap-image-upload my-8" tabIndex={0} onClick={handleClick}>
      {!hasFiles && (
      ...
      {hasFiles && (
        <div className="tiptap-image-upload-previews flex flex-col gap-3">
          {fileItems.length > 1 && (
            <div className="tiptap-image-upload-header flex items-center justify-between py-2 border-b border-border mb-2">
