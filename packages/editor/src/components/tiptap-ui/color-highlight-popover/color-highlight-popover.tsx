...
// --- Hooks ---
import { useMenuNavigation, useIsBreakpoint, cn } from '@nicenote/ui'
...
export const ColorHighlightPopoverButton = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, children, ...props }, ref) => (
    <Button
      ...
      {...props}
    >
      {children ?? <Highlighter className={cn('w-4 h-4 shrink-0', 'tiptap-button-icon')} />}
    </Button>
  )
)
...
export function ColorHighlightPopoverContent({
  ...
  return (
    ...
    >
      <CardBody className={cn(isMobile && 'p-0')} style={isMobile ? { padding: 0 } : {}}>
        <CardItemGroup orientation="horizontal">
          ...
          <ButtonGroup orientation="horizontal">
            <Button
              ...
              role="menuitem"
              data-style="ghost"
              data-active-state={selectedIndex === colors.length ? 'on' : 'off'}
            >
              <Ban className={cn('w-4 h-4 shrink-0', 'tiptap-button-icon')} />
            </Button>
          </ButtonGroup>
        </CardItemGroup>
      </CardBody>
